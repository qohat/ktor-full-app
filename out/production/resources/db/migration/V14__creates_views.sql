...
total_supplies_amounts AS (
  SELECT bill_return_id, SUM(total) as total, SUM(global_amount) as global_amount FROM (
      SELECT brs.bill_return_id,
        CASE
            WHEN u.name = 'Ton' THEN
                CASE
                    WHEN brs.value / GREATEST(brs.quantity * 1000, 0.00000001) > sp.price THEN (SUM(sp.price) * (brs.quantity * 1000))
                    ELSE SUM(brs.value / GREATEST((brs.quantity * 1000), 0.00000001)) * (brs.quantity * 1000)
                END
            WHEN u.name = 'gr' THEN
                CASE
                    WHEN brs.value / GREATEST(brs.quantity / 1000, 0.00000001) > sp.price  THEN SUM(sp.price) * (brs.quantity / 1000)
            ...
            WHEN u.name = 'kg' THEN
                CASE
                    WHEN brs.value / GREATEST(brs.quantity, 0.00000001) > sp.price  THEN SUM(sp.price) * brs.quantity
                    ELSE SUM(brs.value / GREATEST(brs.quantity, 0.00000001)) * brs.quantity
                END
        END as total, sum(brs.value) as global_amount
      FROM bill_returns_supplies brs
      JOIN bill_returns br ON brs.bill_return_id = br.id
      JOIN supplies sp on brs.supply_id = sp.id
      JOIN lists u on u.id = brs.unit
      GROUP BY brs.bill_return_id, brs.value, sp.price, brs.quantity, u.name
...
SUM(tsa.global_amount) as total_supplies_amount,
